/**
 * aux Console worklet — channel-strip asymmetric saturation.
 *
 * Message protocol (main → worklet):
 *   { type: 'set-params', driveDb, character, mix }
...
// ──────────────────────────────────────────────────────────────────────────
// Inlined from packages/dsp-console/wasm/console.js — wasm-bindgen 0.2.122.
// DO NOT EDIT BY HAND — re-bundle from wasm/console.js.
// ──────────────────────────────────────────────────────────────────────────
...
class Console {
    ...
    constructor(sample_rate) {
        const ret = wasm.console_new(sample_rate);
        this.__wbg_ptr = ret;
    ...
    process_stereo(left, right) {
        ...
        wasm.console_process_stereo(this.__wbg_ptr, ptr0, len0, addHeapObject(left), ptr1, len1, addHeapObject(right));
    }
    reset() {
        wasm.console_reset(this.__wbg_ptr);
    }
    set_bypassed(bypassed) {
        wasm.console_set_bypassed(this.__wbg_ptr, bypassed);
    }
    set_params(drive_db, character, mix) {
        wasm.console_set_params(this.__wbg_ptr, drive_db, character, mix);
    }
}
...
class ConsoleProcessor extends AudioWorkletProcessor {
  constructor(options) {
    super();
    const wasmInit = options?.processorOptions?.wasmModule ?? options?.processorOptions?.wasmBytes;
    if (!wasmInit) {
      throw new Error('console-worklet: missing processorOptions.wasmModule or wasmBytes');
    }
    initSync({ module: wasmInit });
    this.console = new Console(sampleRate);
  ...
  onMessage(msg) {
    ...
    switch (msg.type) {
      case 'set-params':
        this.console.set_params(msg.driveDb, msg.character, msg.mix);
        break;
      case 'set-bypassed':
        this.console.set_bypassed(!!msg.bypassed);
        break;
      case 'reset':
        this.console.reset();
        break;
  ...
  process(inputs, outputs) {
    ...
    this.console.process_stereo(outL, outR ?? outL);
    return true;
  }
}

registerProcessor('aux-console-processor', ConsoleProcessor);
