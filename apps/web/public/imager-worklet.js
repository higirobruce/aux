/**
 * aux Imager worklet — M/S stereo width.
 *
...
// ──────────────────────────────────────────────────────────────────────────
// Inlined from packages/dsp-imager/wasm/imager.js — wasm-bindgen 0.2.122.
// DO NOT EDIT BY HAND — re-bundle from wasm/imager.js.
// ──────────────────────────────────────────────────────────────────────────
...
class Imager {
    ...
    constructor(_sample_rate) {
        const ret = wasm.imager_new(_sample_rate);
        this.__wbg_ptr = ret;
    ...
    process_stereo(left, right) {
        ...
        wasm.imager_process_stereo(this.__wbg_ptr, ptr0, len0, addHeapObject(left), ptr1, len1, addHeapObject(right));
    }
    reset() {
        wasm.imager_reset(this.__wbg_ptr);
    }
    set_bypassed(bypassed) {
        wasm.imager_set_bypassed(this.__wbg_ptr, bypassed);
    }
    set_width(width) {
        wasm.imager_set_width(this.__wbg_ptr, width);
    }
}
...
class ImagerProcessor extends AudioWorkletProcessor {
  constructor(options) {
    ...
    if (!wasmInit) {
      throw new Error('imager-worklet: missing processorOptions.wasmModule or wasmBytes');
    }
    initSync({ module: wasmInit });
    this.imager = new Imager(sampleRate);
  ...
  onMessage(msg) {
    ...
    switch (msg.type) {
      case 'set-width':
        this.imager.set_width(msg.width);
        break;
      case 'set-bypassed':
        this.imager.set_bypassed(!!msg.bypassed);
        break;
      case 'reset':
        this.imager.reset();
        break;
  ...
  process(inputs, outputs) {
    ...
    this.imager.process_stereo(outL, outR ?? outL);
    return true;
  }
}

registerProcessor('aux-imager-processor', ImagerProcessor);
